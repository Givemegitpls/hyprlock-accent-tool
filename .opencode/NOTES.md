...
- Multi-output (реализовано): `awww query` парсится в (output, path) — формат строки `: HDMI-A-1: ... currently displaying: image: /path`. Один уникальный путь → старый env-режим (конфиг юзера не трогается, нулевой риск). ≥2 путей → генерация: `src/config.rs` берёт `~/.config/hypr/hyprlock.conf` как шаблон, подставляет $WALLPAPER/$accent/$foreground/$y_offset литералами и вставляет `monitor = <output>` в каждый блок виджета (существующий monitor= заменяется), склеивает рендеры и пишет `/tmp/hyprlock-accent-$USER.conf`, запуск `hyprlock -c`. `--print-config` печатает конфиг вместо запуска. Фолбекы: имена output не распарсились или шаблона нет → single-режим с warning. `--set-offset` при multi → ошибка, просит HYPRLOCK_WALLPAPER. env-оверрайды HYPRLOCK_ACCENT/FOREGROUND применяются ко всем мониторам. НЕ проверено живым локом: поведение дублированного input-field на двух мониторах (вики hyprlock молчит, буфер пароля — состояние сессии).
